...
# Build neural network - FIXED for TF 2.x
def build_network(scope):
    # Use name_scope instead of variable_scope
    with tf.name_scope(scope):
        # Updated initializer for TF 2.19
...
class SarsaWorker(Worker):
    def _collect_experience_and_update(self):
        ...
        while not done:
            ...
            next_action_probs, _ = self.local_model(next_state_tensor)
            ...
            with tf.GradientTape() as tape:
                ...
                next_action_probs, next_value = self.local_model(next_state_tensor)
                ...
                action_oh = tf.reshape(action_oh, [1, N_A])  # Ensure batch dimension
            ...
            grads = [g if g is not None else tf.zeros_like(v)
                    for g, v in zip(grads, self.local_model.trainable_weights)]

            # Apply gradients to global model (all at once)
    ...
    def _collect_experience_and_update(self):
        ...
        while not done:
            ...
            if done:
                ...
                next_state_tensor = np.array([state], dtype=np.float32)
            ...
            with tf.GradientTape() as tape:
                ...
                for i in range(len(buffer_s)):
                    state_tensor = np.array([buffer_s[i]], dtype=np.float32)
                    action_probs, value = self.local_model(state_tensor)
...
# Asynchronous n-step Q-Learning Worker
class NStepQLearningWorker1(Worker):
    def _collect_experience_and_update(self):
        state, _ = self.env.reset()
        done = False
